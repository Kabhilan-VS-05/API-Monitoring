document.addEventListener("DOMContentLoaded", () => {
  ...
  async function populateTimeline() {
    ...
    try {
      ...
      notificationTimeline.innerHTML = data.map(entry => {
        ...
        return `<div class="timeline-entry ${statusColor}"><span>${time} â€” ${entry.channel} reply: ${entry.response || 'Alert delivered'} (${entry.api_id})</span><span>${entry.phone_number || 'Unknown'}</span></div>`;
      }).join('');
    } catch (error) {
          ...
          <article class="training-history-entry">
            <header>
              <div>
                ...
                <small>${escapeHtml(status)} Â· ${escapeHtml(riskLevel)}</small>
              </div>
              <div>
  ...
  async function loadAIInsights(apiId, clickEvent = null) {
    try {
      const triggerEvent = clickEvent || (typeof window !== 'undefined' ? window.event : null);
      // Disable the button to prevent multiple clicks
      const aiButton = triggerEvent?.target?.closest('.button-ai-insights');
      if (aiButton) {
      ...
      const expansionPanel = document.getElementById(`ai-expansion-${apiId}`);
      const expansionContent = document.getElementById(`ai-expansion-content-${apiId}`);
      let insightsTarget = aiInsightsContent;
      ...
      if (expansionPanel && expansionContent) {
        ...
        insightsTarget = expansionContent;
      } else {
        // Fallback to modal if expansion panel not found
      ...
      if (!insightsTarget) {
        throw new Error('AI insights container not found');
      }
      ...
      insightsTarget.innerHTML = `
        <div style="text-align: center; padding: 1.25rem;">
          ...
          <h3 style="margin-top: 1rem; color: #58A6FF;"><i class="ri-brain-line"></i> AI Model Training</h3>
          <p style="color: #8B949E; margin-top: 0.35rem;">Starting AI workflow...</p>
          <div data-training-status style="margin-top: 1rem; color: #58A6FF; font-weight: 600; font-size: 1.1rem;"></div>
          <div style="margin-top: 0.75rem; text-align: left; max-width: 420px; margin-left: auto; margin-right: auto;">
            <div data-training-step="fetch" class="training-stage-item active"><i class="ri-database-2-line"></i> Fetching historical data</div>
            <div data-training-step="train" class="training-stage-item"><i class="ri-brain-line"></i> Training model</div>
            <div data-training-step="analyze" class="training-stage-item"><i class="ri-line-chart-line"></i> Analyzing risk patterns</div>
            <div data-training-step="finalize" class="training-stage-item"><i class="ri-checkbox-circle-line"></i> Finalizing results</div>
          </div>
          
          <!-- Progress Bar -->
          <div class="training-progress-container" style="margin-top: 1rem;">
            <div class="training-progress-bar">
              <div data-training-progress-fill class="training-progress-fill"></div>
            </div>
            <div data-training-percentage style="margin-top: 0.5rem; color: #8B949E; font-size: 0.9rem;">0%</div>
          </div>
          
          <p data-training-eta style="color: #D29922; font-size: 0.9rem; margin-top: 0.85rem; font-weight: 600;">
            <i class="ri-time-line"></i> Estimated remaining: ~2-3 minutes
          </p>
          <p style="color: #8B949E; font-size: 0.8rem; margin-top: 0.35rem;">
      ...
      if (!document.getElementById('spinner-style')) {
        ...
        style.textContent = `
          ...
          .training-stage-item.active {
            color: #58A6FF;
            background: rgba(88, 166, 255, 0.12);
            font-weight: 600;
          }

          .training-stage-item.done {
            color: #3FB950;
          }
        `;
      ...
      const statusEl = insightsTarget.querySelector('[data-training-status]');
      const progressFill = insightsTarget.querySelector('[data-training-progress-fill]');
      const progressPercentage = insightsTarget.querySelector('[data-training-percentage]');
      const trainingEta = insightsTarget.querySelector('[data-training-eta]');

      const trainingSteps = {
        fetch: insightsTarget.querySelector('[data-training-step="fetch"]'),
        train: insightsTarget.querySelector('[data-training-step="train"]'),
        analyze: insightsTarget.querySelector('[data-training-step="analyze"]'),
        finalize: insightsTarget.querySelector('[data-training-step="finalize"]')
      };

      const setTrainingStep = (activeStep) => {
        const order = ['fetch', 'train', 'analyze', 'finalize'];
        const activeIndex = order.indexOf(activeStep);
        order.forEach((step, idx) => {
          const el = trainingSteps[step];
          if (!el) return;
      ...
      const updateEtaText = (progressValue, elapsedMs) => {
        if (!trainingEta) return;
        if (progressValue >= 100) {
          trainingEta.innerHTML = '<i class="ri-time-line"></i> Estimated remaining: almost done';
          return;
        }
        if (progressValue <= 5) {
          trainingEta.innerHTML = '<i class="ri-time-line"></i> Estimated remaining: ~2-3 minutes';
          return;
        }

        const remainingMs = (elapsedMs / Math.max(progressValue, 1)) * (100 - progressValue);
        const remainingSec = Math.max(10, Math.min(240, Math.round(remainingMs / 1000)));
        if (remainingSec >= 120) {
          trainingEta.innerHTML = `<i class="ri-time-line"></i> Estimated remaining: ~${Math.ceil(remainingSec / 60)} minutes`;
        } else {
          trainingEta.innerHTML = `<i class="ri-time-line"></i> Estimated remaining: ~${remainingSec} seconds`;
        }
      };
      ...
      setTrainingStep('fetch');

      // Simulated progress animation (smooth increase while waiting for real updates)
      let simulatedProgress = 5;

      const simulateProgress = setInterval(() => {
        if (simulatedProgress < 90 && !(progressFill && progressFill.dataset.realUpdate)) {
      ...
      const trainingPromise = new Promise(async (resolve, reject) => {
        ...
        const MIN_DISPLAY_TIME = 0; // Show result as soon as backend reports completion
        ...
        const pollStatus = setInterval(async () => {
          try {
            ...
            if (statusResponse.ok) {
              ...
              const hasBackendProgress = Number.isFinite(rawProgress) && rawProgress >= 0;
              ...
              if (statusEl && progressFill && progressPercentage) {
                ...
                if (currentStatus === 'starting') {
                  setTrainingStep('fetch');
                  const displayProgress = hasBackendProgress ? Math.max(10, Math.min(25, Math.floor(rawProgress))) : 15;
                  statusEl.innerHTML = `<i class="ri-loader-4-line"></i> ${statusMessage || 'Fetching historical data...'}`;
                  progressFill.style.width = displayProgress + '%';
                  progressPercentage.textContent = displayProgress + '%';
                  updateEtaText(displayProgress, elapsedTime);
                } else if (currentStatus === 'training') {
                  const fallbackTrainingProgress = Math.min(20 + (elapsedTime / 1000), 75);
                  const progressPercent = hasBackendProgress
                    ? Math.max(20, Math.min(90, Math.floor(rawProgress)))
                    : Math.floor(fallbackTrainingProgress);
                  const stageMessage = statusMessage || (
                    stage.includes('lstm') ? 'Training LSTM model...' :
                    stage.includes('autoencoder') ? 'Training Autoencoder model...' :
                    stage.includes('preparing') ? 'Preparing training dataset...' :
                    'Training AI model...'
                  );

                  // Show different messages based on progress
                  if (progressPercent < 35 || stage.includes('preparing')) {
                    setTrainingStep('fetch');
                  } else if (progressPercent < 80 || stage.includes('lstm') || stage.includes('autoencoder')) {
                    setTrainingStep('train');
                  } else {
                    setTrainingStep('analyze');
                  }

                  statusEl.innerHTML = `<i class="ri-loader-4-line"></i> ${stageMessage}`;
                  progressFill.style.width = progressPercent + '%';
                  progressPercentage.textContent = progressPercent + '%';
                  updateEtaText(progressPercent, elapsedTime);
                } else if (currentStatus === 'analyzing') {
                  setTrainingStep('analyze');
                  const displayProgress = hasBackendProgress ? Math.max(85, Math.min(95, Math.floor(rawProgress))) : 85;
                  statusEl.innerHTML = `<i class="ri-loader-4-line"></i> ${statusMessage || 'Analyzing risk patterns...'}`;
                  progressFill.style.width = displayProgress + '%';
                  progressPercentage.textContent = displayProgress + '%';
                  updateEtaText(displayProgress, elapsedTime);
                  ...
                  clearInterval(simulateProgress);
                  setTrainingStep('finalize');
                  Object.values(trainingSteps).forEach((el) => {
                    if (!el) return;
                  ...
                  if (trainingEta) {
                    trainingEta.innerHTML = '<i class="ri-checkbox-circle-line"></i> Training finished. Fetching AI results...';
                  }

                  const totalElapsed = Date.now() - trainingStartTime;
                  const remainingTime = Math.max(0, MIN_DISPLAY_TIME - totalElapsed);
                  setTimeout(() => resolve(true), remainingTime);
      ...
      try {
        ...
        trainingFailureMessage = error?.message ? String(error.message) : 'Training unavailable';
      }

      // Now fetch prediction, anomalies, and insight history in parallel
      const [prediction, anomalies, insights, insightHistory] = await Promise.all([
      ...
      // Normalize API results to avoid rendering failures on partial/null responses
      ...
      const riskScoreRaw = Number(safePrediction.risk_score);
      const riskScore = Number.isFinite(riskScoreRaw)
        ? Math.max(0, Math.min(100, Math.round(riskScoreRaw)))
        : 0;
      const confidenceValue = Number(safePrediction.confidence);
      const confidencePct = Number.isFinite(confidenceValue)
        ? `${Math.round(confidenceValue * 100)}%`
        : 'N/A';
      const modelName = safePrediction.model || 'LSTM + Autoencoder';
      const modelAccuracyText = typeof safePrediction.model_accuracy === 'number'
        ? `${(safePrediction.model_accuracy * 100).toFixed(1)}%`
        : 'N/A';
      const modelAucText = typeof safePrediction.model_auc === 'number'
        ? safePrediction.model_auc.toFixed(3)
        : 'N/A';
      const predictionReason = safePrediction.reason
        ? String(safePrediction.reason)
        : 'AI prediction is currently unavailable.';
      const willFail = safePrediction.will_fail === true;
      ...
      let nextTrainTime = new Date(Date.now() + trainingIntervalMinutes * 60 * 1000);
      let lastTrainedText = 'Not available';
      if (safePrediction.last_trained) {
        const lastTrainedTime = new Date(safePrediction.last_trained);
        if (!Number.isNaN(lastTrainedTime.getTime())) {
          nextTrainTime = new Date(lastTrainedTime.getTime() + trainingIntervalMinutes * 60 * 1000);
          lastTrainedText = lastTrainedTime.toLocaleString();
        }
      }

      const getCountdownLabel = (targetTime) => {
        const msRemaining = targetTime.getTime() - Date.now();
        if (msRemaining <= 0) return 'Training now';
      ...
      html += `
        <div class="ai-overview-strip">
          <div class="ai-overview-item">
            <small>Next Train</small>
            <strong data-training-countdown>${escapeHtml(getCountdownLabel(nextTrainTime))}</strong>
          </div>
          <div class="ai-overview-item">
            <small>Model</small>
          ...
          <div class="ai-overview-item">
            ...
            <strong>${escapeHtml(lastTrainedText)}</strong>
          </div>
          <button class="button-secondary ai-history-btn" onclick="showTrainingHistory('${apiId}')">
            <i class="ri-history-line"></i> Training History
          </button>
        </div>
      `;

      const riskLevel = riskScore > 70 ? 'danger' : riskScore > 40 ? 'warning' : 'safe';
      const riskText = willFail
        ? '<i class="ri-alert-line"></i> High Risk'
        : riskScore > 40
          ? '<i class="ri-flashlight-line"></i> Moderate Risk'
          : '<i class="ri-checkbox-circle-line"></i> Low Risk';
        ...
        <div class="ai-prediction-card">
          ...
          <div class="risk-meter">
            ...
            <div class="risk-label">
              ...
              <span><strong>${riskScore}/100</strong></span>
            </div>
          </div>
          <div class="prediction-details">
            <div style="margin-bottom: 0.35rem;"><strong>Confidence:</strong> ${escapeHtml(confidencePct)}</div>
            <div style="margin-bottom: 0.6rem;"><strong>Analysis:</strong> ${escapeHtml(predictionReason)}</div>
            <div class="ai-model-metrics compact">
      ...
      if (trainingFailureMessage) {
        html += `
          <div class="insight-card warning">
            <div class="insight-title"><i class="ri-alert-line"></i> Training Update</div>
            <div class="insight-message">
              Latest training did not complete (${escapeHtml(trainingFailureMessage)}). Showing last available AI results.
            </div>
          </div>
        `;
      }
      ...
      if (insightsList.length > 0) {
        insightsList.slice(0, 3).forEach((insight) => {
          const insightType = ['info', 'success', 'warning', 'error'].includes(insight?.type) ? insight.type : 'info';
          const insightTitle = insight?.title || 'AI Insight';
          const insightMessage = insight?.message || 'No details available.';
          const insightDetails = insight?.details ? String(insight.details) : '';
          const insightAction = insight?.action ? String(insight.action) : '';
          html += `
            <div class="insight-card ${insightType}">
              <div class="insight-title">${escapeHtml(insightTitle)}</div>
              <div class="insight-message">${escapeHtml(insightMessage)}</div>
              ${insightDetails ? `<div class="insight-message">${escapeHtml(insightDetails)}</div>` : ''}
              ${insightAction ? `<div class="insight-action"><i class="ri-lightbulb-line"></i> ${escapeHtml(insightAction)}</div>` : ''}
            </div>
          `;
        });
      }

      // Previous AI insights summary
      if (insightHistoryList.length > 1) {
        const previous = insightHistoryList[1] || {};
        const createdAt = previous.created_at ? new Date(previous.created_at).toLocaleString() : 'Unknown';
        const level = previous.risk_level ? previous.risk_level.toUpperCase() : 'UNKNOWN';
        const score = typeof previous.risk_score === 'number' ? Math.round(previous.risk_score) : null;
        const summary = previous.summary ? String(previous.summary) : 'No previous summary available.';
        html += `
          <div class="insight-history-card compact">
            <div class="insight-title"><i class="ri-time-line"></i> Previous AI Insight</div>
            <div class="insight-history-header">
              <span class="insight-history-time">${escapeHtml(createdAt)}</span>
              <span class="insight-history-badge">${escapeHtml(level)}${score !== null ? ` - ${score}/100` : ''}</span>
            </div>
      ...
      if (anomaliesList.length > 0) {
        html += `
          <div class="insight-card error">
            <div class="insight-title"><i class="ri-alert-line"></i> Anomalies Detected (${anomaliesList.length})</div>
            <div class="anomaly-list">
        `;
        anomaliesList.slice(0, 3).forEach((anomaly) => {
          const anomalyType = String(anomaly?.type || 'unknown').replace(/_/g, ' ').toUpperCase();
          const anomalyDescription = anomaly?.description ? String(anomaly.description) : 'No description available.';
          const severityRaw = String(anomaly?.severity || 'medium').toLowerCase();
          const anomalySeverity = ['critical', 'high', 'medium', 'low'].includes(severityRaw) ? severityRaw : 'medium';
          html += `
            <div class="anomaly-item ${anomalySeverity}">
              <strong>${escapeHtml(anomalyType)}:</strong> ${escapeHtml(anomalyDescription)}
            </div>
          `;
        });
        html += `</div></div>`;
      ...
      if (insightsList.length === 0 && anomaliesList.length === 0 && !willFail) {
        html += `
      ...
      if (insightsTarget) {
        insightsTarget.innerHTML = html;
      }
      if (expansionPanel && expansionPanel.classList.contains('active')) {
      ...
      // Start countdown timer in the active insights container
      const countdownTarget = insightsTarget?.querySelector('[data-training-countdown]');
      startTrainingCountdown(nextTrainTime, countdownTarget);

    } catch (error) {
      console.error('Error loading AI insights:', error);
      const expansionContent = document.getElementById(`ai-expansion-content-${apiId}`);
      if (expansionContent) {
        expansionContent.innerHTML = '<p class="placeholder error">Failed to load AI insights</p>';
      } else if (aiInsightsContent) {
  ...
  function startTrainingCountdown(targetTime, countdownEl = null) {
    if (!countdownEl) {
      countdownEl = document.querySelector('[data-training-countdown]');
    }
    ...
    const updateTimer = () => {
      ...
      if (msRemaining <= 0) {
        countdownEl.textContent = 'Training now';
  ...
  function renderMonitorItem(monitor) {
    ...
    return `
          <div class="monitor-card-wrapper" id="monitor-wrapper-${monitor.id}">
            <div class="monitor-item" data-id="${monitor.id}" onclick="showDetailsView('${monitor.id}')">
                ...
                <div class="monitor-actions-container">
                    ...
                    <button class="button-ai-insights" onclick="event.stopPropagation(); loadAIInsights('${monitor.id}', event)" title="AI Prediction">
            ...
            <div id="ai-expansion-${monitor.id}" class="ai-expansion-panel">
                <div id="ai-expansion-content-${monitor.id}" class="ai-expansion-content">
                    <!-- Loaded dynamically -->
                </div>
            </div>
          </div>
  ...
  function normalizeFrequencyValue(freq) {
    ...
    // Only snap if difference is within tolerance (0.001 minutes â‰ˆ 0.06s)
    if (minDiff <= 0.001) {
      return closest;
    }
