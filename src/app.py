...
from modules import auth_routes, notification_routes, monitor_routes, github_routes, alert_routes, ai_routes
                        ...
                        if current_status != "Low Network":
                            ...
                            if alert_result:
                                if alert_result.get("success"):
                                    ...
                                    print(f"[Alert] Downtime/Recovery alert failed: {alert_result.get('error', 'Unknown error')}")
...
def register_modular_routes():
    """Register domain route modules once after core helpers are initialized."""
    global _routes_registered

    deps = {key: value for key, value in globals().items() if not key.startswith("__")}
    auth_routes.register_routes(app, deps)
    notification_routes.register_routes(app, deps)
    monitor_routes.register_routes(app, deps)
    github_routes.register_routes(app, deps)
    alert_routes.register_routes(app, deps)
    ai_routes.register_routes(app, deps)
    _routes_registered = True

register_modular_routes()
