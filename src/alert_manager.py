...
class AlertManager:
    def __init__(self, mongo_db):
        ...
        # Create downtime issue on first real failure by default.
        self.failure_threshold = max(1, int(os.getenv("ALERT_FAILURE_THRESHOLD", "1")))
        # Close downtime issues after this many consecutive healthy checks.
        self.recovery_threshold = max(1, int(os.getenv("ALERT_RECOVERY_THRESHOLD", "3")))
    ...
    def _get_github_settings(self, user_id):
        """
        Resolve GitHub settings for an API owner.
        Falls back to the most recently updated settings to support legacy
        monitors that were created before per-user ownership was enforced.
        """
        settings = self.db.github_settings.find_one({"user_id": user_id})
        if settings:
            return settings

        fallback = self.db.github_settings.find_one(
            {
                "repo_owner": {"$exists": True, "$ne": ""},
                "repo_name": {"$exists": True, "$ne": ""},
            },
            sort=[("updated_at", -1)],
        )
        if fallback:
            print(
                f"[Alert] No GitHub settings for user={user_id}; "
                f"using fallback settings from user={fallback.get('user_id')}"
            )
        return fallback
    ...
    def should_create_alert(self, api_id, current_status, api_url=None):
        """
        Create an alert on failure threshold, keep a single open alert
        during repeated failures, and allow a new alert after recovery.
        """
    ...
    def should_create_recovery_alert(self, api_id):
        ...
        print(f"[Alert] API {api_id} consecutive successes: {consecutive_successes}/{self.recovery_threshold}")
        ...
        print(f"[Alert] Not enough successes yet ({consecutive_successes}/{self.recovery_threshold})")
        return False, None
    ...
    def create_downtime_alert(self, api_id, api_url, reason):
        ...
        user_id = self._api_owner_id(api_id)
        settings = self._get_github_settings(user_id)
        if not settings:
        ...
        if result.get("success"):
            ...
            print(f"[Alert] Failed to create downtime alert for {api_url}: {error_message}")
